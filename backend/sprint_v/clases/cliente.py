...
class ClientNotAuthorizedError(Exception):
    pass
...
class Cliente:
    ...
    def puede_retirar(self, monto, limiteDiario):
        if monto < 0:
            raise ValueError("No puede retirar un monto negativo")
        
        self.resetear_monto_diario()
        
        if self.montoRetiradoHoy + monto > limiteDiario:
            raise DailyLimitExceededError(f"No puede retirar más de ${limiteDiario} por día.")
...
# Clases de tipo de cliente heredando de Cliente (con excepciones)
...
class ClienteClassic(Cliente):
    ...
    def retirar_dinero(self, cuenta, monto):
        return super().retirar_dinero(cuenta, monto)

    def alta_tarjeta_credito(self, tarjetaCredito):
        raise Exception("No puede tener tarjeta de crédito siendo cliente Classic")
...
class ClienteGold(Cliente):
    ...
    def realizar_transferencia(self, clienteDestino, monto):
        if self.cajaDeAhorroPesos.saldo >= monto:
            self.cajaDeAhorroPesos.saldo -= monto
            clienteDestino.cajaDeAhorroPesos.saldo += monto
            return 1
        else:
            raise InsufficientFundsError("Saldo insuficiente para realizar la transferencia.")
...
class ClienteBlack(Cliente):
    ...
    def __init__(self, numero, nombre, apellido, dni, tarjetaDebito, direccion, cajaAhorroPesos, cajaAhorroDolares):
        ...
        self.cajaAhorroDolares = cajaAhorroDolares
        self.tarjetaCredito = []
        self.comision = 1  # Sin comisiones para clientes Black
        self.autorizacion = True
